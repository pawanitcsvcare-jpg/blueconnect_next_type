// @ts-nocheck — legacy Apex chart config; gradual typing deferred
...
export const activityOptions = {
  ...
  legend: {
    ...
    formatter: function (name: string, ctx: { w: { globals: { series: number[][] } }; seriesIndex: number }) {
      const sum = (ctx.w.globals.series[ctx.seriesIndex] ?? []).reduce((acc: number, v: number) => acc + v, 0);
      return `${name} (${sum})`;
    }
  },
  grid: {
  ...
  tooltip: {
    ...
    custom: function (opts: any) {
      ...
      const rows = (w.config.series || []).map(function (s: { name?: string }, idx: number) {
...
export const buildActivityLineConfig = (activationData, deactivationData, categories) => {
  return {
    ...
    options: {
      ...activityOptions,
      xaxis: { ...activityOptions.xaxis, categories },
    } as ApexOptions,
  };
};
...
export const buildUsageBarConfig = (seriesMap, categories) => {
  ...
  return { series, options: options as ApexOptions };
};
